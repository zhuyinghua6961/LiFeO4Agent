...
- [x] **4.1.1** 创建对话蓝图
  - [x] 创建 Blueprint：`conversation_bp`
  - [x] 设置 URL 前缀：`/api/conversations`
  - [x] 配置 CORS

- [x] **4.1.2** 实现 `POST /api/conversations` - 创建新对话
  - [x] 获取请求体（user_id, title）
  - [x] 验证请求参数
  - [x] 调用 `ConversationService.create_conversation()`
  - [x] 返回 201 状态码和对话详情
  - [x] 处理异常并返回错误响应

- [x] **4.1.3** 实现 `GET /api/conversations` - 获取对话列表
  - [x] 获取查询参数（user_id, page, page_size）
  - [x] 验证用户ID
  - [x] 调用 `ConversationService.get_conversation_list()`
  - [x] 返回 200 状态码和对话列表
  - [x] 处理异常并返回错误响应

- [x] **4.1.4** 实现 `GET /api/conversations/{id}` - 获取对话详情
  - [x] 获取路径参数（conversation_id）
  - [x] 获取查询参数（user_id）
  - [x] 验证参数
  - [x] 调用 `ConversationService.get_conversation_detail()`
  - [x] 返回 200 状态码和完整对话
  - [x] 处理对话不存在的情况（404）
  - [x] 处理权限错误（403）

- [x] **4.1.5** 实现 `POST /api/conversations/{id}/messages` - 添加消息
  - [x] 获取路径参数（conversation_id）
  - [x] 获取请求体（user_id, message）
  - [x] 验证消息格式（role, content, steps, references）
  - [x] 调用 `ConversationService.add_message()`
  - [x] 返回 201 状态码
  - [x] 处理异常并返回错误响应

- [x] **4.1.6** 实现 `PUT /api/conversations/{id}` - 更新对话标题
  - [x] 获取路径参数（conversation_id）
  - [x] 获取请求体（user_id, title）
  - [x] 验证参数
  - [x] 调用 `ConversationService.update_conversation_title()`
  - [x] 返回 200 状态码
  - [x] 处理异常并返回错误响应

- [x] **4.1.7** 实现 `DELETE /api/conversations/{id}` - 删除对话
  - [x] 获取路径参数（conversation_id）
  - [x] 获取查询参数（user_id）
  - [x] 验证参数
  - [x] 调用 `ConversationService.delete_conversation()`
  - [x] 返回 204 状态码（无内容）
  - [x] 处理对话不存在的情况（404）
  - [x] 处理权限错误（403）

- [x] **4.1.8** 注册蓝图到主应用
  - [x] 在 `main.py` 中导入 `conversation_bp`
  - [x] 调用 `app.register_blueprint(conversation_bp)`

#### 4.2 集成到现有问答流程

- [x] **4.2.1** 修改 `POST /api/ask_stream` 接口
  - [x] 接收额外参数：user_id, conversation_id
  - [x] 如果 conversation_id 为空，自动创建新对话
  - [x] 在流式响应开始前，保存用户消息
  - [x] 在流式响应完成后，保存AI回复（包含steps）
  - [x] 保持向后兼容（参数可选）
...
- [x] **5.1.1** 添加对话管理API方法
  - [x] 实现 `createConversation(userId, title)` 方法
  - [x] 实现 `getConversationList(userId, page, pageSize)` 方法
  - [x] 实现 `getConversationDetail(conversationId, userId)` 方法
  - [x] 实现 `addMessage(conversationId, userId, message)` 方法
  - [x] 实现 `updateConversationTitle(conversationId, userId, title)` 方法
  - [x] 实现 `deleteConversation(conversationId, userId)` 方法

- [x] **5.1.2** 修改 `askStream()` 方法
  - [x] 添加 userId 和 conversationId 参数
  - [x] 在请求体中包含这些参数
  - [x] 保持向后兼容
...
- [x] **5.2.1** 添加用户状态管理
  - [x] 添加 `userId` 状态
  - [x] 添加 `setUserId()` 方法
  - [x] 从 localStorage 或登录状态获取 userId

- [x] **5.2.2** 修改对话加载逻辑
  - [x] 修改 `loadChats()` 方法，从服务器加载
  - [x] 调用 `api.getConversationList(userId)`
  - [x] 保留 localStorage 作为离线缓存
  - [x] 实现服务器优先策略

- [x] **5.2.3** 修改对话创建逻辑
  - [x] 修改 `createNewChat()` 方法
  - [x] 调用 `api.createConversation(userId)`
  - [x] 获取服务器返回的 conversation_id
  - [x] 同步到 localStorage

- [x] **5.2.4** 修改消息保存逻辑
  - [x] 修改 `addUserMessage()` 方法
  - [x] 调用 `api.addMessage()` 同步到服务器
  - [x] 保留 localStorage 同步
  - [x] 实现乐观更新（先更新UI，后台同步）

- [x] **5.2.5** 修改对话删除逻辑
  - [x] 修改 `deleteChat()` 方法
  - [x] 调用 `api.deleteConversation()`
  - [x] 同步删除 localStorage
  - [x] 处理删除失败的情况

- [x] **5.2.6** 添加同步状态管理
  - [x] 添加 `syncStatus` 状态（synced/syncing/failed）
  - [x] 添加同步失败重试机制
  - [x] 显示同步状态图标
...
- [x] **5.3.1** 修改发送消息逻辑
  - [x] 在 `sendMessage()` 中传递 userId 和 conversationId
  - [x] 调用修改后的 `api.askStream()`
  - [x] 处理自动创建对话的情况
...
- **2024-01-18 20:45**：完成后端 API 层集成测试
  - ✅ 所有 API 端点测试通过
  - ✅ 步骤（steps）和引用（references）完整保存验证通过
  - ✅ role 字段兼容性处理（bot → assistant）
  - ✅ 用户认证和权限验证正常
  - 📊 总进度：57/85 (67.1%)
- **2024-01-18 20:50**：完成前端改造
  - ✅ API 服务层添加对话管理方法
  - ✅ Store 层实现服务器同步逻辑
  - ✅ UI 层集成新 API 和同步状态显示
  - ✅ 实现服务器优先策略，localStorage 作为离线缓存
  - ✅ 添加同步状态图标（已同步/同步中/同步失败）
  - 📊 总进度：70/85 (82.4%)
